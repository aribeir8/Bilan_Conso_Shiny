...
setwd("~/Master_2/Semestre3/Python_RAvance/R_Avance/Projet") # A MODIFIER
...
ui <- dashboardPage(
    ...
    dashboardHeader(
        title="Bilan de consommation électrique",
        titleWidth=400
    ),
    ...
    dashboardSidebar(
        ...
        sidebarMenu(
            ...
            menuItem("Détails", tabName = "Details")
        ),
        
        selectInput('Maille',
                    'Choisissez la collectivité',
                    choices = c('Moyenne france', 'Région', 'Département', 'Commune')),
    ...
    dashboardBody(
        tabItems(
            ...
            tabItem('Details',
                    ...
                    , downloadButton("downloadData2", "Télécharger")
            )
        )
    )
)
...
server <- function(input, output) {
    ...
    output$ui_moyenne <-  renderUI({
        ...
        if (input$Maille == "Département"){
            ...
            selectInput("moyenne",
                        "Choisissez la ou les moyennes :",
                        choices = c('Moyenne Région', 'Moyenne Département'),
                        multiple = TRUE)
        }
    })
    ...
    output$ui_region <-  renderUI({
        ...
        if (input$Maille == "Région" | input$Maille == "Département" | input$Maille == "Commune"){
    ...
    output$ui_departement <-  renderUI({
        ...
        if (input$Maille == "Département" | input$Maille == "Commune"){
    ...
    filtre_valuebox_tot <- reactive({
        ...
        out_valuebox_tot <- out_valuebox_tot %>%
            ...
            filter(Conso.totale..MWh. != "NA") %>%
            filter(Conso.totale..MWh. != 0) %>%
            summarise(sum(Conso.totale..MWh.))
    ...
    filtre_valuebox_tot2 <- reactive({
        ...
        out_valuebox_tot2 <- mutate(out_valuebox_tot2, prod.totale = Energie.produite.annuelle.Photovoltaïque.Enedis..MWh.+
                                        ...
                                        Energie.produite.annuelle.Cogénération.Enedis..MWh.+
                                        Energie.produite.annuelle.Autres.filières.Enedis..MWh.)
        out_valuebox_tot2 <- out_valuebox_tot2 %>% summarise(sum(prod.totale))
    ...
    filtre2 <- reactive({
        ...
        out2 <- out2 %>%
            ...
            rename(Bio = Energie.produite.annuelle.Bio.Energie.Enedis..MWh.,
                   ...
                   Photovoltaïque = Energie.produite.annuelle.Photovoltaïque.Enedis..MWh.,
                   Cogénération = Energie.produite.annuelle.Cogénération.Enedis..MWh.,
    ...
    filtrePlot2 <- reactive({
        outPlot2 <- filtre2() %>%
            ...
            summarise(Autres = sum(Autres),
                      ...
                      Cogénération = sum(Cogénération),
                      Photovoltaïque = sum(Photovoltaïque),
                      Eolien = sum(Eolien),
                      Bio = sum(Bio)) %>%
            summarise(Autres = round(mean(Autres),2),
                      Hydraulique = round(mean(Hydraulique),2),
                      Cogénération = round(mean(Cogénération),2),
                      Photovoltaïque = round(mean(Photovoltaïque),2),
                      Eolien = round(mean(Eolien),2),
                      Bio = round(mean(Bio),2))
        
        outPlot2 <- t(outPlot2)
        names <- c("Autres","Hydraulique","Cogénération","Photovoltaïque","Eolien","Bio")
        produc <- outPlot2[,1]
    ...
    Action_barplot <- eventReactive(input$action,{
        ...
        ggplot(bar) + 
            ...
            ggtitle("Consommation et production pour une année moyenne de la collectivité")
    ...
    filtre_graph_industrie <- reactive({
        ...
        out_int <- out_int %>% group_by(Année) %>%
    ...
    filtre_graph_agriculture <- reactive({
        ...
        out_int[is.na(out_int)] <- 0
        out_int <- out_int %>% group_by("AGRICULTURE") %>%
            group_by(Année)%>%
            summarise(sum(Conso.totale..MWh.))
        out_int
    })
    ...
    filtre_graph_interactif2_photo <- reactive({
        ...
        out_int2 <- out_int2 %>%
            select(Année, Energie.produite.annuelle.Photovoltaïque.Enedis..MWh.)
        out_int2 <- out_int2 %>% group_by(Année) %>% summarise(a = sum(Energie.produite.annuelle.Photovoltaïque.Enedis..MWh.))
    })
    ...
    filtre_graph_interactif2_eolien <- reactive({
        ...
        out_int2 <- out_int2 %>% group_by(Année) %>% summarise(a = sum(Energie.produite.annuelle.Eolien.Enedis..MWh.))
    })
    ...
    filtre_graph_interactif2_hydro <- reactive({
        ...
        out_int2 <- out_int2 %>% group_by(Année) %>% summarise(a = sum(Energie.produite.annuelle.Hydraulique.Enedis..MWh.))
    })
    ...
    filtre_graph_interactif2_bio <- reactive({
        ...
        out_int2 <- out_int2 %>% group_by(Année) %>% summarise(a = sum(Energie.produite.annuelle.Bio.Energie.Enedis..MWh.))
    })
    ...
    filtre_graph_interactif2_co <- reactive({
        ...
        out_int2 <- out_int2 %>% group_by(Année) %>% summarise(a = sum(Energie.produite.annuelle.Cogénération.Enedis..MWh.))
    })
    ...
    filtre_graph_interactif2_autre <- reactive({
        ...
        out_int2 <- out_int2 %>% group_by(Année) %>% summarise(a = sum(Energie.produite.annuelle.Autres.filières.Enedis..MWh.))
    })
    ...
    Act_evolution <- eventReactive(input$action,{
        ...
        ggplot() +
            geom_line(aes(x = pl_agriculture$Année, y = pl_agriculture$'sum(Conso.totale..MWh.)', color = "Conso agriculture")) +
            geom_line(aes(x = pl_inconnu$Année, y = pl_inconnu$'sum(Conso.totale..MWh.)', color = "Conso inconnu")) +
            geom_line(aes(x = pl_industrie$Année, y = pl_industrie$'sum(Conso.totale..MWh.)', color = "Conso industrie")) +
            geom_line(aes(x = pl_petit$Année, y = pl_petit$'sum(Conso.totale..MWh.)', color = "Conso petit")) +
            geom_line(aes(x = pl_residentiel$Année, y = pl_residentiel$'sum(Conso.totale..MWh.)', color = "Conso residentiel")) +
            geom_line(aes(x = pl_tertiaire$Année, y = pl_tertiaire$'sum(Conso.totale..MWh.)', color = "Conso tertiaire")) +
            geom_line(aes(x = pl2_autre$Année, y = pl2_autre$a, color = "Production autre")) +
            geom_line(aes(x = pl2_co$Année, y = pl2_co$a, color = "Production cogeneration")) +
            geom_line(aes(x = pl2_bio$Année, y = pl2_bio$a, color = "Production bio")) +
            geom_line(aes(x = pl2_hydro$Année, y = pl2_hydro$a, color = "Production hydraulique")) +
            geom_line(aes(x = pl2_eolien$Année, y = pl2_eolien$a, color = "Production eolien")) +
            geom_line(aes(x = pl2_photo$Année, y = pl2_photo$a, color = "Production photo")) +
            ggtitle("Evolution de la consommation et de la production au fil des années") +
            ylab("MWh") + xlab("Année")
    ...
    filtre_data_conso <- reactive({
        ...
        if(input$Maille == "Commune"){
            ...
            data_conso <- conso_com %>%
                ...
                summarise(Conso_tot_moy_commune = mean(round(Conso.totale..MWh., 2))) %>%
                rename(Secteur = CODE.GRAND.SECTEUR)
            
            if ("Moyenne Département" %in% input$moyenne){
                ...
                moyenne_departement <- conso_com %>%
                    ...
                    filter(Conso.totale..MWh. != "NA") %>%
                    group_by(CODE.GRAND.SECTEUR) %>%
                    summarise(Conso_tot_moy_communes_in_dep = mean(round(Conso.totale..MWh., 2))) %>%
                    select(Conso_tot_moy_communes_in_dep)
                
                data_conso <- cbind(data_conso, moyenne_departement)
                
            }
            
            if ("Moyenne Région" %in% input$moyenne){
                ...
                moyenne_region <- conso_com %>%
                    ...
                    filter(Conso.totale..MWh. != "NA") %>%
                    group_by(CODE.GRAND.SECTEUR) %>%
                    summarise(Conso_tot_moy_communes_in_reg = mean(round(Conso.totale..MWh., 2))) %>%
                    select(Conso_tot_moy_communes_in_reg)
            ...
            moyenne_france <- conso_com %>%
                ...
                filter(Conso.totale..MWh. != "NA") %>%
                group_by(CODE.GRAND.SECTEUR) %>%
                summarise(Moyenne_france = mean(round(Conso.totale..MWh., 2))) %>%
                select(Moyenne_france)
            
            data_conso <- cbind(data_conso, moyenne_france)
            
            data_conso <- mutate(data_conso, ecart_moy_pourcent = -100 + Conso_tot_moy_commune*100/Moyenne_france)
            ...
            data_conso <- conso_dep %>%
                ...
                filter(Conso.totale..MWh. != "NA") %>%
                group_by(CODE.GRAND.SECTEUR) %>%
                summarise(Conso_tot_moy_dep = mean(round(Conso.totale..MWh., 2))) %>%
                rename(Secteur = CODE.GRAND.SECTEUR)
            ...
            data_conso <- mutate(data_conso, ecart_moy_pourcent = -100 + Conso_tot_moy_dep*100/Moyenne_france)
            ...
            data_conso <- conso_reg %>%
                ...
                filter(Conso.totale..MWh. != "NA") %>%
                group_by(CODE.GRAND.SECTEUR) %>%
                summarise(Conso_tot_moy_reg = mean(round(Conso.totale..MWh., 2))) %>%
                rename(Secteur = CODE.GRAND.SECTEUR)
            ...
            data_conso <- mutate(data_conso, ecart_moy_pourcent = -100 + Conso_tot_moy_reg*100/Moyenne_france)
    ...
    filtre_data_prod <- reactive({
        ...
        if(input$Maille == "Commune"){
            ...
            data_prod <- prod_com %>%
                ...
                rename(Photovoltaïque = Energie.produite.annuelle.Photovoltaïque.Enedis..MWh.) %>%
                ...
                rename(Bio = Energie.produite.annuelle.Bio.Energie.Enedis..MWh.) %>%
                rename(Cogénération = Energie.produite.annuelle.Cogénération.Enedis..MWh.) %>%
                rename(Autres = Energie.produite.annuelle.Autres.filières.Enedis..MWh.) %>%
                summarise(Photovoltaïque = mean(round(Photovoltaïque, 2)),
                          Eolien = mean(round(Eolien, 2)),
                          Hydraulique = mean(round(Hydraulique, 2)),
                          Bio = mean(round(Bio, 2)),
                          Cogénération = mean(round(Cogénération, 2)),
                          Autres = mean(round(Autres, 2)))
            ...
            data_prod <- mutate(data.frame(data_prod),
                                ecart_moy_pourcent = -100 + as.numeric(prod_moy_com)*100/as.numeric(Moyenne_france))
            ...
            data_prod <- mutate(data.frame(data_prod), 
                                ecart_moy_pourcent = -100 + as.numeric(prod_moy_dep)*100/as.numeric(Moyenne_france))
            ...
            data_prod <- mutate(data.frame(data_prod),
                                ecart_moy_pourcent = -100 + as.numeric(prod_moy_reg)*100/as.numeric(Moyenne_france))
